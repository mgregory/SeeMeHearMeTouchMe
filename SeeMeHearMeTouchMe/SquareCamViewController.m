...
@interface SquareCamViewController () <GLKViewDelegate>
...
- (BOOL)writeCGImageToCameraRoll:(CGImageRef)cgImage withMetadata:(NSDictionary *)metadata
{
	...
	[library writeImageDataToSavedPhotosAlbum:(__bridge_transfer id)destinationData metadata:metadata completionBlock:^(NSURL *assetURL, NSError *error) {
		if (destinationData)
			...
			if (error) {
				...
				if (doingFaceDetection) {
                    ...
                    dispatch_sync(videoDataOutputQueue, ^(void) {
						...
						[self writeCGImageToCameraRoll:cgImageResult withMetadata:(__bridge_transfer id)attachments];
						if (attachments)
							CFRelease(attachments);
						if (cgImageResult)
...
//	ciImage = [gloom valueForKey: @"outputImage"];
//
	
	
	
    CIImage *image = [CIImage imageWithCVPixelBuffer:pixelBuffer];
	image = [CIFilter filterWithName:@"CIFalseColor" keysAndValues:kCIInputImageKey, image, @"inputColor0", [CIColor colorWithRed:0.0 green:0.2 blue:0.0], @"inputColor1", [CIColor colorWithRed:0.0 green:0.0 blue:1.0], nil].outputImage;
	...
	[hueAdjust setValue: [NSNumber numberWithFloat: 208.094]
				 forKey: @"inputAngle"];
	...
	[coreImageContext  drawImage: image atPoint: CGPointZero fromRect: r];	
	
	//[coreImageContext render:ciImage toCVPixelBuffer:pixelBuffer bounds:r colorSpace:CGColorSpaceCreateDeviceRGB()];
...
- (void)viewDidLoad
{
    ...
    GLKView *view = (GLKView *)previewView;
